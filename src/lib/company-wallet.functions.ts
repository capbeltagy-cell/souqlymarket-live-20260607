...
export const getMyCompanyFundingWallet = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: company } = await supabase
      .from("companies").select("id, name_ar, name_en").eq("owner_id", userId).maybeSingle();
    if (!company) return { company: null, wallet: null, transactions: [] };

    const { data: existing } = await supabase
      .from("wallets").select("*").eq("user_id", userId).eq("kind", "company_funding" as never).maybeSingle();
...
export const listMyCompanyDeposits = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      .from("company_deposits" as never).select("*").order("created_at", { ascending: false }).limit(100);
    if (error) throw new Error(error.message);
...
export const cancelMyCompanyDeposit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { error } = await context.supabase
      .from("company_deposits" as never).update({ status: "cancelled" } as never)
...
export const adminListCompanyDeposits = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => z.object({
    status: z.enum(["pending", "under_review", "approved", "rejected", "cancelled", "all"]).default("pending"),
  }).parse(d))
  .handler(async ({ context, data }) => {
...
export const adminReviewCompanyDeposit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.action === "approve") {
      const { error } = await context.supabase.rpc("approve_company_deposit" as never, {
        _deposit_id: data.id, _admin_notes: data.admin_notes ?? null,
      } as never);
      if (error) throw new Error(error.message);
    } else {
      const { error } = await context.supabase.rpc("reject_company_deposit" as never, {
        _deposit_id: data.id, _admin_notes: data.admin_notes ?? null,
      } as never);
      if (error) throw new Error(error.message);
    }
...
export const activateListingPromotion = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: c } = await supabase
      .from("companies").select("id").eq("id", l.company_id).eq("owner_id", userId).maybeSingle();
    if (!c) throw new Error("Not authorized");

    // Persist campaign caps if provided
    const patch: Record<string, unknown> = {};
    if (data.campaign_max_conversions !== undefined) patch.campaign_max_conversions = data.campaign_max_conversions;
    if (data.campaign_budget_egp !== undefined) patch.campaign_budget_egp = data.campaign_budget_egp;
    if (Object.keys(patch).length > 0) {
      const { error: uErr } = await supabase.from("listings").update(patch as never).eq("id", data.listing_id);
      if (uErr) throw new Error(uErr.message);
    }

    const { data: res, error } = await supabase.rpc("activate_listing_promotion" as never, {
...
export const deactivateListingPromotion = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { data: res, error } = await context.supabase.rpc("deactivate_listing_promotion" as never, {
      _listing_id: data.listing_id, _status: data.status,
    } as never);
    if (error) throw new Error(error.message);
    return { ok: true, result: res };
  });
...
export const previewListingReserve = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { data: res, error } = await context.supabase.rpc("compute_listing_required_reserve" as never, {
      _listing_id: data.listing_id,
    } as never);
    if (error) throw new Error(error.message);
