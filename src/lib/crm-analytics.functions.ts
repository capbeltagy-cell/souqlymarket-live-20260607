...
export const getAgentPerformance = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const conversionRate =
      leadsGenerated > 0 ? Math.round((conversions / leadsGenerated) * 1000) / 10 : 0;
    const commRows = (comms.data ?? []) as any[];
    const totalCommissions = commRows.reduce((s, c) => s + Number(c.amount ?? 0), 0);
    const pendingCommissions = commRows
      .filter((c) => c.status === "pending")
      .reduce((s, c) => s + Number(c.amount ?? 0), 0);
    const paidCommissions = commRows
...
export const getMyReferralAnalytics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: ag } = await supabase
      ...
      .maybeSingle();
    if (!ag)
      return { clicks: 0, registrations: 0, conversions: 0, revenue: 0, currency: "EGP" as const };
    ...
    const conversions = refRows.reduce((s, r) => s + (r.conversions ?? 0), 0);
    const registrations = ((companyRefs.data ?? []) as any[]).reduce(
      (s, r) => s + (r.signups ?? 0),
      0,
    );
    const revenue = ((comms.data ?? []) as any[]).reduce((s, c) => s + Number(c.amount ?? 0), 0);
...
export const getAdminExecutiveDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [
      ...
      supabaseAdmin
        .from("companies")
        .select("id, subscription_plan, subscription_expires_at, category_slug"),
    ...
    const subscriptionRevenue = ((subsPayments.data ?? []) as any[]).reduce(
      (s, p) => s + Number(p.amount ?? 0),
      0,
    );
    const pendingPayouts = ((commsPending.data ?? []) as any[]).reduce(
