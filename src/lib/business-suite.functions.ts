  ...
  .handler(async ({ context }) => {
    ...
    const [contacts, inventory, invoices, sales, purchases, suppliers] = await Promise.all([
      ...
      client
        ...
        .select("id, total, paid_amount, status", { count: "exact" })
        .eq("company_id", companyId),
      client
        .from("business_sales_orders")
    ...
    return {
      ...
      totals: {
        ...
        lowStock: stockRows.filter(
          (row: any) => Number(row.quantity) <= Number(row.minimum_quantity),
        ).length,
        invoices: invoices.count ?? 0,
        outstanding: invoiceRows.reduce(
          (sum: number, row: any) => sum + Math.max(0, Number(row.total) - Number(row.paid_amount)),
          0,
        ),
        salesOrders: sales.count ?? 0,
        salesValue: (sales.data ?? []).reduce(
          (sum: number, row: any) => sum + Number(row.total || 0),
          0,
        ),
        purchaseOrders: purchases.count ?? 0,
        purchaseValue: (purchases.data ?? []).reduce(
          (sum: number, row: any) => sum + Number(row.total || 0),
          0,
        ),
...
export const listBusinessContacts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const companyId = await getCompany(context as AuthContext);
    const { data, error } = await db(context as AuthContext)
      .from("crm_contacts")
...
export const createBusinessContact = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const companyId = await getCompany(context as AuthContext);
    const { error } = await db(context as AuthContext)
      .from("crm_contacts")
      .insert({
...
export const listInventoryItems = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await db(context as AuthContext)
      .from("inventory_items")
      .select(
        "id, name, sku, barcode, quantity, reserved_quantity, minimum_quantity, unit_cost, unit_price, currency, is_active, created_at",
      )
      .eq("company_id", companyId)
      .order("created_at", { ascending: false })
      .limit(500);
...
export const listBusinessInvoices = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await db(context as AuthContext)
      .from("business_invoices")
      .select(
        "id, invoice_number, status, issue_date, due_date, currency, total, paid_amount, notes, crm_contacts(full_name)",
      )
      .eq("company_id", companyId)
      .order("created_at", { ascending: false })
      .limit(250);
...
export const listPurchaseOrders = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await db(context as AuthContext)
      .from("business_purchase_orders")
      .select(
        "id, order_number, status, order_date, expected_date, currency, total, business_suppliers(name)",
      )
      .eq("company_id", companyId)
      .order("created_at", { ascending: false })
      .limit(250);
