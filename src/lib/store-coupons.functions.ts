...
async function assertStoreOwner(supabase: any, userId: string, store_id: string) {
  const { data } = await supabase.from("stores").select("id, owner_id").eq("id", store_id).maybeSingle();
...
export const listCoupons = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: items } = await (context.supabase.from("store_coupons" as never) as any)
      .select("*").eq("store_id", data.store_id).order("created_at", { ascending: false });
    return { items: items ?? [] };
  });
...
export const deleteCoupon = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => z.object({ id: z.string().uuid(), store_id: z.string().uuid() }).parse(d))
  .handler(async ({ context, data }) => {
    await assertStoreOwner(context.supabase, context.userId, data.store_id);
...
export const validateCoupon = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { data: coupon } = await (context.supabase.from("store_coupons" as never) as any)
      .select("*").eq("store_id", data.store_id).eq("code", data.code.toUpperCase()).eq("active", true).maybeSingle();
    if (!coupon) throw new Error("كوبون غير صالح");
    const now = Date.now();
    if (coupon.starts_at && new Date(coupon.starts_at).getTime() > now) throw new Error("الكوبون لم يبدأ بعد");
    if (coupon.ends_at && new Date(coupon.ends_at).getTime() < now) throw new Error("انتهت صلاحية الكوبون");
    if (data.subtotal < Number(coupon.min_order || 0)) throw new Error(`الحد الأدنى للطلب ${coupon.min_order} ج.م`);
    if (coupon.usage_limit_total && coupon.used_count >= coupon.usage_limit_total) throw new Error("انتهى استخدام الكوبون");
    ...
    if ((userUsed ?? 0) >= Number(coupon.usage_limit_per_user || 1)) throw new Error("لقد استخدمت هذا الكوبون من قبل");
