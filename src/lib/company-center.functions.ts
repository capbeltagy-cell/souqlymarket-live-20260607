...
export const getCompanyCommandCenter = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<CommandCenterPayload> => {
    ...
    const { data: company } = await supabase
      .from("companies")
      .select(
        "id, name_ar, name_en, logo_url, cover_url, description_ar, description_en, governorate, city, phone, email, website, industry, subscription_plan, subscription_expires_at, is_verified",
      )
      .eq("owner_id", userId)
      .maybeSingle();
    ...
    if (!company) {
      return {
        ...
        kpis: {
          ...
          totalLeads: 0,
          newLeads: 0,
          activePromotedListings: 0,
          activeCampaigns: 0,
          participatingMarketers: 0,
          referralClicks: 0,
          referralConversions: 0,
          conversionRate: 0,
          totalCommissionsEgp: 0,
          pendingCommissionsEgp: 0,
        },
        checklist: [],
        activity: [],
      };
    ...
    const activePromotedListings = L.filter(
      (l) =>
        l.marketer_promotion_enabled &&
        (l.promotion_status ?? "active") === "active" &&
        l.status === "approved",
    ).length;
    ...
    if (listingIds.length > 0) {
      const { data: refs } = await supabase
        .from("referrals")
        .select("agent_id, clicks, conversions")
    ...
    const conversionRate =
      referralClicks > 0 ? Math.round((referralConversions / referralClicks) * 1000) / 10 : 0;
    ...
    const pendingCommissionsEgp = C.filter((c) => c.status === "pending").reduce(
    ...
    const checklist = [
      {
        key: "created",
        label_ar: "إنشاء الشركة",
        label_en: "Company created",
        done: true,
        ctaTo: "/company",
      },
      {
        key: "logo",
        label_ar: "رفع الشعار",
        label_en: "Upload logo",
        done: has((company as any).logo_url),
        ctaTo: "/company",
      },
      {
        key: "cover",
        label_ar: "رفع صورة الغلاف",
        label_en: "Upload cover image",
        done: has((company as any).cover_url),
        ctaTo: "/company",
      },
      {
        key: "description",
        label_ar: "كتابة وصف الشركة",
        label_en: "Company description",
        done: has((company as any).description_ar) || has((company as any).description_en),
        ctaTo: "/company",
      },
      {
        key: "governorate",
        label_ar: "اختيار المحافظة",
        label_en: "Governorate selected",
        done: has((company as any).governorate),
        ctaTo: "/company",
      },
      {
        key: "city",
        label_ar: "اختيار المدينة",
        label_en: "City selected",
        done: has((company as any).city),
        ctaTo: "/company",
      },
      {
        key: "contact",
        label_ar: "معلومات التواصل",
        label_en: "Contact information",
        done: has((company as any).phone) || has((company as any).email),
        ctaTo: "/company",
      },
      {
        key: "first_product",
        label_ar: "نشر أول منتج",
        label_en: "Publish first product",
        done: activeProducts > 0,
        ctaTo: "/listings/new",
      },
      {
        key: "first_service",
        label_ar: "نشر أول خدمة",
        label_en: "Publish first service",
        done: activeServices > 0,
        ctaTo: "/listings/new",
      },
      {
        key: "first_promotion",
        label_ar: "تفعيل أول عرض للمسوقين",
        label_en: "Activate first marketer promotion",
        done: activePromotedListings > 0,
        ctaTo: "/company-campaigns",
      },
      {
        key: "subscription",
        label_ar: "تفعيل الاشتراك المدفوع",
        label_en: "Activate paid subscription",
        done: isPaid,
        ctaTo: "/subscribe",
      },
    ];
    ...
    for (const l of recentLeads ?? []) {
      activity.push({
        ...
        kind: "lead",
        at: l.created_at,
        title_ar: `عميل محتمل جديد: ${l.buyer_name ?? "غير معروف"}`,
        title_en: `New lead: ${l.buyer_name ?? "unknown"}`,
        link: "/leads",
      });
    }
    ...
    return {
      ...
      kpis: {
        ...
        newLeads: newLeads ?? 0,
        activePromotedListings,
        activeCampaigns,
        participatingMarketers,
        referralClicks,
        referralConversions,
        conversionRate,
        totalCommissionsEgp,
        pendingCommissionsEgp,
      },
      checklist,
...
export const listCompanyCampaigns = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<{ hasCompany: boolean; rows: CompanyCampaignRow[] }> => {
    ...
    const { data: listings } = await supabase
      .from("listings")
      .select(
        "id, title_ar, title_en, type, status, marketer_promotion_enabled, promotion_status, commission_type, commission_percentage, commission_fixed_amount, conversion_goal, promotion_conditions, leads_count",
      )
      .eq("company_id", cid)
    ...
    const [{ data: refs }, { data: camps }, { data: comms }] = await Promise.all([
      supabase
        .from("referrals")
        .select("listing_id, agent_id, clicks, conversions")
        .in("listing_id", ids),
    ...
    const rows: CompanyCampaignRow[] = L.map((l) => {
      ...
      return {
        ...
        conversion_goal: l.conversion_goal,
        promotion_conditions: l.promotion_conditions,
        participatingMarketers: new Set(R.map((r) => r.agent_id)).size,
        activeCampaigns: CA.filter((c) => c.status === "active").length,
        clicks,
        leads: Number(l.leads_count ?? 0),
        conversions,
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.status === "active") {
      const { error } = await supabase.rpc(
        ...
        { _listing_id: data.id } as never,
      );
      if (error) throw new Error(error.message);
    } else {
      const { error } = await supabase.rpc(
        "deactivate_listing_promotion" as never,
        { _listing_id: data.id, _status: data.status } as never,
      );
      if (error) throw new Error(error.message);
    }
    return { ok: true };
