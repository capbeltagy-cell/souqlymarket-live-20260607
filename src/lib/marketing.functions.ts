...
export const getCampaignAnalytics = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [{ data: refs }, { data: camp }] = await Promise.all([
      ...
      supabase
        .from("agent_campaigns")
        .select("*, listings(title_ar, title_en)")
        .eq("id", data.id)
        .maybeSingle(),
    ]);
    const rows = refs ?? [];
...
export const requestWithdrawal = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: settings } = await (supabaseAdmin.from("platform_settings" as never) as any)
...
export const getPlatformSettings = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    const { data, error } = await (context.supabase.from("platform_settings" as never) as any)
      .select("*")
      .maybeSingle();
    if (error) throw new Error(error.message);
  ...
  .inputValidator((d: unknown) =>
    z
      .object({
        ...
        platform: z
          .enum(["facebook", "instagram", "tiktok", "whatsapp", "x", "linkedin"])
          .default("facebook"),
        locale: z.enum(["ar", "en"]).default("ar"),
...
export const generateProductPromotion = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (!product && data.listingId) {
      ...
      if (l) {
        const title =
          (data.locale === "ar" ? l.title_ar : l.title_en) ?? l.title_ar ?? l.title_en ?? "";
