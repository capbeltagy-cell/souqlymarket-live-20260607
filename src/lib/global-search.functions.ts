...
export const globalSearch = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [companiesR, listingsR, factoriesR, wholesaleR, rfqsR, tendersR, agentsR] =
      await Promise.all([
        supabaseAdmin
          .from("companies")
          .select(
        ...
        supabaseAdmin
          .from("agents")
          .select(
            "id, user_id, headline_ar, headline_en, city, country, is_verified, is_premium, is_trusted, created_at",
          )
          .or(
            `headline_ar.ilike.${like},headline_en.ilike.${like},bio_ar.ilike.${like},bio_en.ilike.${like}`,
          )
          .limit(lim * 2),
      ]);
    ...
    const agents = rankAgents((agentsR.data ?? []) as any[])
      .map((a: any) => ({ ...a, profile: pMap.get(a.user_id) ?? null }))
      .slice(0, lim);
