...
export function ListingCard({ l }: { l: ListingCardData }) {
  ...
  const company =
    (locale === "ar" ? l.companies?.name_ar : l.companies?.name_en) ??
    l.companies?.name_en ??
    l.companies?.name_ar ??
    "";
    ...
    <div className="group overflow-hidden rounded-3xl premium-panel transition-all duration-300 hover:-translate-y-1 hover:shadow-gold hover:border-primary/40">
      <div className="relative aspect-[4/3] overflow-hidden bg-surface-2">
        ...
        {l.featured && (!l.featured_until || new Date(l.featured_until).getTime() > Date.now()) && (
          <Badge className="absolute top-3 end-3 bg-primary text-primary-foreground hover:bg-primary font-semibold tracking-wider">
            ★ {t("feature_featured_listings")}
          </Badge>
        )}
        <button
          onClick={toggleFav}
          ...
          {company && (
            <div className="text-xs text-muted-foreground mt-1 inline-flex flex-wrap items-center gap-2">
              {l.companies?.is_verified && (
                <Badge variant="secondary" className="text-[10px] tracking-[0.18em] px-2 py-1">
                  {t("verified_company")}
                </Badge>
              )}
              <span>
          ...
          <div className="flex gap-1.5 flex-shrink-0">
            ...
            {wa && (
              <Button
                ...
                size="sm"
                variant="outline"
                className="text-success border-success/40 hover:bg-success/5 hover:text-success px-2"
