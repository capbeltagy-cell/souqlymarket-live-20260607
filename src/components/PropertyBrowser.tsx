...
export function PropertyBrowser({ listingType, subtypes, titleAr, titleEn }: Props) {
  ...
  useEffect(() => {
    ...
    supabase
      .from("listings")
      .select(
        "id, type, title_ar, title_en, images, price, currency, country, city, governorate, area_sqm, property_subtype, purpose, commission_percentage, featured, featured_until, company_id, companies(name_ar, name_en, is_verified)",
      )
      .eq("type", listingType)
      .eq("status", "approved")
  ...
  const filtered = useMemo(
    () =>
      rows.filter((r) => {
        ...
        if (
          governorate !== "all" &&
          normalizeEgyptGovernorate(r.governorate ?? null) !== governorate
        )
          return false;
    ...
    <div className="min-h-screen flex flex-col bg-surface-2">
      ...
      <div className="container-souqly py-8 flex-1">
        ...
        <p className="text-muted-foreground mb-6">
          {ar
            ? "تصفح حسب النوع والمحافظة والسعر والمساحة"
            : "Browse by type, governorate, price and area"}
        </p>

        <div className="flex flex-wrap gap-2 mb-6">
          <Button
            size="sm"
            variant={subtype === "all" ? "default" : "outline"}
          ...
          {subtypes.map((s) => (
            <Button
              key={s.value}
              size="sm"
              variant={subtype === s.value ? "default" : "outline"}
          ...
          <div className="ms-auto flex gap-1">
            {(["all", "sale", "rent"] as const).map((p) => (
              <Button
                key={p}
                size="sm"
                variant={purposeFilter === p ? "default" : "outline"}
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mb-6 p-4 rounded-lg border border-border bg-card">
          ...
          <Input
            placeholder={ar ? "أدنى سعر" : "Min price"}
            value={minPrice}
            onChange={(e) => setMinPrice(e.target.value)}
            type="number"
          />
          <Input
            placeholder={ar ? "أقصى سعر" : "Max price"}
            value={maxPrice}
            onChange={(e) => setMaxPrice(e.target.value)}
            type="number"
          />
          <Input
            placeholder={ar ? "أدنى مساحة (م²)" : "Min area (m²)"}
            value={minArea}
                    ...
                    {r.property_subtype && (
                      <Badge variant="secondary" className="text-[10px]">
                        {subtypes.find((s) => s.value === r.property_subtype)
                          ? ar
                            ? subtypes.find((s) => s.value === r.property_subtype)!.label_ar
                            : subtypes.find((s) => s.value === r.property_subtype)!.label_en
                          : r.property_subtype}
                      </Badge>
                    )}
                    {r.area_sqm && (
                      <span className="inline-flex items-center gap-1">
                        ...
                        {Number(r.area_sqm).toLocaleString(ar ? "ar-EG" : "en-US")} م²
                      </span>
                    )}
                  </div>
                )}
