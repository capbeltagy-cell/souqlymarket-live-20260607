...
export function ImageUploader({
  ...
  const uploadOne = useCallback(
    async (file: File, source: "uploaded" | "live_capture"): Promise<UploadedImage | null> => {
      if (!user) {
        toast.error(locale === "ar" ? "سجّل الدخول أولاً" : "Sign in first");
        return null;
      }
      if (!ALLOWED_IMAGE_TYPES.has(file.type)) {
        toast.error(locale === "ar" ? "صيغة الصورة غير مسموحة" : "Unsupported image format");
        return null;
      }
      if (file.size > MAX_SOURCE_BYTES) {
        toast.error(
          locale === "ar"
            ? "حجم الصورة يجب ألا يتجاوز 12 ميجابايت"
      ...
      try {
        ...
        const toUpload = await imageCompression(file, COMPRESS_OPTS);
  ...
  const handleFiles = useCallback(
    async (files: FileList | File[], source: "uploaded" | "live_capture" = "uploaded") => {
              ...
              >
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition bg-black/40 flex items-center justify-center gap-1">
                  <button
                    ...
                    onClick={() => move(i, i - 1)}
                    disabled={i === 0}
                    className="text-white text-xs px-1.5 py-0.5 rounded bg-white/20 disabled:opacity-40"
                  >
                    ‹
                  </button>
                ...
                <button
                  type="button"
                  onClick={() => remove(i)}
                  className="absolute -top-1 -end-1 h-5 w-5 rounded-full bg-destructive text-destructive-foreground grid place-items-center shadow"
                >
                  <X className="h-3 w-3" />
                </button>
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            <label className="inline-flex items-center gap-2 h-10 px-4 rounded-md bg-primary text-primary-foreground text-sm font-medium cursor-pointer hover:bg-primary-hover">
              ...
              <input
                ...
                multiple
                className="hidden"
                onChange={(e) => e.target.files && handleFiles(e.target.files, "uploaded")}
              />
            </label>
            <label className="inline-flex items-center gap-2 h-10 px-4 rounded-md bg-success text-success-foreground text-sm font-medium cursor-pointer hover:opacity-90">
              ...
              <input
                ...
                capture="environment"
                className="hidden"
                onChange={(e) => e.target.files && handleFiles(e.target.files, "live_capture")}
              />
            </label>
            <div className="flex items-center text-xs text-muted-foreground">
              {locale === "ar"
                ? `أو اسحب الصور هنا • ${value.length}/${max}`
                : `or drag & drop • ${value.length}/${max}`}
            </div>
          </div>
        )}
        {!canAddMore && (
...
export function toLegacyShape(imgs: UploadedImage[]): {
  images: string[];
  image_sources: ("uploaded" | "live_capture")[];
} {
  return { images: imgs.map((i) => i.url), image_sources: imgs.map((i) => i.source) };
}
export function fromLegacyShape(
  images: string[] = [],
  sources: ("uploaded" | "live_capture")[] = [],
): UploadedImage[] {
