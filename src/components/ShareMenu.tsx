...
import { Copy, Share2, MessageCircle, Send, Linkedin, Facebook, Twitter, Instagram, Link2 } from "lucide-react";
...
export function ShareMenu({
  url, caption, title, variant = "button", earning, triggerLabel, className,
}: ShareMenuProps) {
  ...
  const copy = async (text: string, msg: string) => {
    ...
    catch { toast.error(ar ? "تعذر النسخ" : "Copy failed"); }
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md" dir={ar ? "rtl" : "ltr"}>
        <DialogHeader>
          ...
          <DialogDescription>
            {earning
              ? (ar ? "شارك برابطك واربح عند حدوث تحويل مؤهل." : "Share your link and earn on qualified conversions.")
              : (ar ? "شارك هذا الإعلان" : "Share this item")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            />
            <Button size="sm" variant="outline" className="gap-1" onClick={() => copy(shareUrl, ar ? "تم نسخ الرابط" : "Link copied")}>
              <Link2 className="h-3.5 w-3.5" />{ar ? "نسخ الرابط" : "Copy link"}
            </Button>
          </div>

          <Textarea readOnly value={caption} rows={5} className="text-sm resize-none" />
          <div className="flex flex-wrap gap-2">
            <Button size="sm" variant="outline" className="gap-1" onClick={() => copy(caption, ar ? "تم نسخ النص" : "Caption copied")}>
              <Copy className="h-3.5 w-3.5" />{ar ? "نسخ النص" : "Copy caption"}
            </Button>
            <Button size="sm" variant="outline" className="gap-1" onClick={() => copy(fullText, ar ? "تم نسخ النص والرابط" : "Text + link copied")}>
              <Copy className="h-3.5 w-3.5" />{ar ? "نسخ النص + الرابط" : "Copy text + link"}
            </Button>
            <Button size="sm" variant="outline" className="gap-1" onClick={native}>
              <Share2 className="h-3.5 w-3.5" />{ar ? "مشاركة النظام" : "System share"}
            </Button>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-2">
            <SocialBtn href={links.whatsapp} icon={MessageCircle} label="WhatsApp" color="text-[#25D366]" />
            <SocialBtn href={links.telegram} icon={Send}          label="Telegram" color="text-[#229ED9]" />
            <SocialBtn href={links.facebook} icon={Facebook}      label="Facebook" color="text-[#1877F2]" />
            <SocialBtn href={links.twitter}  icon={Twitter}       label="X / Twitter" color="text-foreground" />
            <SocialBtn href={links.linkedin} icon={Linkedin}      label="LinkedIn" color="text-[#0A66C2]" />
            <Button size="sm" variant="outline" className="gap-1 justify-start" onClick={() => copy(fullText, ar ? "انسخ ثم انشره على إنستجرام أو تيك توك" : "Paste on Instagram / TikTok")}>
              <Instagram className="h-4 w-4 text-[#E4405F]" />IG / TikTok
            </Button>
          </div>
...
function SocialBtn({ href, icon: Icon, label, color }: { href: string; icon: typeof MessageCircle; label: string; color: string }) {
  return (
