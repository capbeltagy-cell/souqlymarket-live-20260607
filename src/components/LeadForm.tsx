...
export function LeadForm({ listingId }: { listingId: string }) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDone(true);
      toast.success(ar ? "تم إرسال طلبك. ستتواصل معك الشركة قريباً." : "Request sent. The company will contact you shortly.");
    } catch (err) {
  ...
  if (done) {
    return (
      <div className="rounded-xl border border-success/40 bg-success/5 p-5 text-sm">
        <div className="font-semibold text-success mb-1">{ar ? "تم استلام طلبك ✓" : "Request received ✓"}</div>
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-xl border border-border bg-card p-5 shadow-card space-y-3">
      <div>
        <h3 className="font-semibold">{ar ? "طلب معلومات / عرض سعر" : "Request information / quote"}</h3>
      ...
      <div className="space-y-1.5">
        ...
        <Input id="lead-name" required maxLength={120} value={name} onChange={(e) => setName(e.target.value)} dir={ar ? "rtl" : "ltr"} />
      </div>
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="space-y-1.5">
          ...
          <Input id="lead-phone" type="tel" maxLength={40} value={phone} onChange={(e) => setPhone(e.target.value)} />
        </div>
      </div>
      <div className="space-y-1.5">
        <Label htmlFor="lead-msg">{ar ? "الرسالة" : "Message"}</Label>
        <Textarea id="lead-msg" rows={3} maxLength={2000} value={message} onChange={(e) => setMessage(e.target.value)} dir={ar ? "rtl" : "ltr"} />
      </div>
      <p className="text-[11px] text-muted-foreground">
        {ar ? "يجب توفير البريد أو الهاتف على الأقل." : "Please provide at least an email or phone."}
      </p>
      <Button type="submit" disabled={busy} className="w-full bg-primary hover:bg-primary-hover gap-2">
