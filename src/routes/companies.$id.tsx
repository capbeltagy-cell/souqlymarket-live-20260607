...
export const Route = createFileRoute("/companies/$id")({
  ...
  head: ({ loaderData, params }) => {
    ...
    const desc = (
      m?.description_en ??
      m?.description_ar ??
      `${m?.industry ?? "B2B"} company on Souqly.`
    ).slice(0, 160);
  ...
  notFoundComponent: () => (
    <Shell>
      <div className="p-10 text-center">Company not found</div>
    </Shell>
  ),
  errorComponent: () => (
    <Shell>
    ...
    (async () => {
      ...
      if (data) {
        const { data: items } = await supabase
          .from("listings")
          .select(
            "id, type, title_ar, title_en, images, price, currency, country, commission_percentage, featured, company_id, companies(name_ar, name_en)",
          )
          .eq("company_id", id)
          .eq("status", "approved")
    ...
    <Shell>
      <section
        ...
        style={
          company.cover_url
            ? {
                backgroundImage: `linear-gradient(rgba(0,0,0,.55),rgba(0,0,0,.55)), url(${company.cover_url})`,
        ...
        <div className="container-souqly py-10 flex flex-col md:flex-row md:items-center gap-6">
          {company.logo_url ? (
            <img
              ...
              alt=""
              className="h-24 w-24 rounded-lg object-cover bg-primary-foreground/15"
            />
          ) : (
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 mt-3 text-sm opacity-90 flex-wrap">
              ...
              <Badge variant="secondary">
                {listings.length} {t("listings_count")}
              </Badge>
            </div>
          </div>
          <div className="flex flex-col gap-2">
            ...
            {!contact.phone && !contact.email && (
              <Button asChild variant="secondary" className="gap-2">
                <Link to="/auth">
                  <MessageCircle className="h-4 w-4" />
                  {locale === "ar" ? "سجّل الدخول للتواصل" : "Sign in to contact"}
                </Link>
              </Button>
            )}
            <ShareMenu
      ...
      <section className="container-souqly py-8 flex-1 grid lg:grid-cols-3 gap-6">
        <aside className="lg:col-span-1 space-y-4">
            ...
            <div className="mt-4 space-y-2 text-sm">
              ...
              {!contact.website && !contact.email && !contact.phone && (
                <div className="text-muted-foreground text-xs">
                  {locale === "ar"
                    ? "سجّل الدخول لرؤية بيانات التواصل."
          ...
          <Button asChild variant="outline" className="w-full">
            <Link to="/companies">← {t("nav_companies")}</Link>
          </Button>
        </aside>
