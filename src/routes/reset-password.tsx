      ...
      <div className="w-full max-w-md rounded-xl border border-border bg-card p-8 shadow-card">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input id="pw" type="password" minLength={6} required className="mt-1.5" value={pw} onChange={(e) => setPw(e.target.value)} />
          </div>
          <div>
            <Label htmlFor="pw2">{t("reset_confirm_password")}</Label>
            <Input id="pw2" type="password" minLength={6} required className="mt-1.5" value={pw2} onChange={(e) => setPw2(e.target.value)} />
          </div>
          <Button type="submit" disabled={busy || !ready} className="w-full bg-primary hover:bg-primary-hover">
            {t("reset_submit")}
          </Button>
