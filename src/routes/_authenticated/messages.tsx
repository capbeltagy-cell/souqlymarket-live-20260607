...
function MessagesPage() {
  ...
  useEffect(() => {
    loadConvs(); /* eslint-disable-next-line */
  }, [user?.id]);
  useEffect(() => {
    if (activeId) loadMsgs(activeId); /* eslint-disable-next-line */
  }, [activeId]);
  ...
  const otherId =
    active && user ? (active.buyer_id === user.id ? active.seller_id : active.buyer_id) : null;
                ...
                >
                  ...
                  {!recording ? (
                    ...
                    <button
                      ...
                      className="p-2 rounded-md bg-destructive text-destructive-foreground animate-pulse"
                      title="إيقاف التسجيل"
                    >
      ...
      >
        ...
        {m.attachment_type === "pdf" && m.attachment_url && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className={`flex items-center gap-2 rounded-md p-2 mb-1 ${mine ? "bg-primary-foreground/20" : "bg-background"}`}
          >
            <FileText className="h-4 w-4" />
            <span className="truncate max-w-40">{m.attachment_name ?? "PDF"}</span>
          </a>
        )}
        {m.attachment_type === "file" && m.attachment_url && (
          ...
          >
            <Paperclip className="h-4 w-4" />
            <span className="truncate max-w-40">{m.attachment_name ?? "file"}</span>
          </a>
        )}
        {m.attachment_type === "voice" && m.attachment_url && (
          <div className="flex items-center gap-2 mb-1">
            <button
              onClick={togglePlay}
              type="button"
              className={`h-8 w-8 rounded-full grid place-items-center ${mine ? "bg-primary-foreground/20" : "bg-background"}`}
            >
          ...
          if (qm)
            return (
              <Link
                to="/quotations/$id"
                params={{ id: qm[1] }}
                className={`block rounded-md p-3 mb-1 border ${mine ? "bg-primary-foreground/10 border-primary-foreground/30" : "bg-background border-border"}`}
              >
                <div className="flex items-center gap-2 font-semibold mb-1">
                  <FileSignature className="h-4 w-4" /> عرض سعر
                </div>
                <div className="text-xs opacity-80">{qm[2] || "افتح للعرض والتفاصيل"}</div>
                <div className={`text-[10px] mt-1 ${mine ? "opacity-80" : "text-primary"}`}>
                  افتح العرض ←
                </div>
              </Link>
            );
        ...
        <div
          className={`text-[10px] mt-1 flex items-center gap-1 ${mine ? "opacity-80" : "text-muted-foreground"}`}
        >
