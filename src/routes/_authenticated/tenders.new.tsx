...
function NewTender() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    try {
      const r = await createTender({ data: { title, description: desc || undefined, category_slug: cat || undefined, governorate: gov || undefined, budget: budget ? Number(budget) : undefined, deadline: deadline || undefined } });
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="container-souqly py-10 flex-1 max-w-2xl">
        ...
        <form onSubmit={submit} className="space-y-3">
          <Input required maxLength={200} placeholder={ar ? "عنوان المناقصة" : "Tender title"} value={title} onChange={(e) => setTitle(e.target.value)} />
          <Textarea rows={6} placeholder={ar ? "نطاق العمل والمتطلبات" : "Scope and requirements"} value={desc} onChange={(e) => setDesc(e.target.value)} />
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <Input type="number" min="0" placeholder={ar ? "الميزانية (ج.م)" : "Budget (EGP)"} value={budget} onChange={(e) => setBudget(e.target.value)} />
