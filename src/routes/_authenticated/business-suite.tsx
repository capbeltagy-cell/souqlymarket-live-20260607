...
function BusinessSuitePage() {
  ...
  const lowStockIds = useMemo(
    () =>
      new Set(
        items
          .filter((item) => Number(item.quantity) <= Number(item.minimum_quantity))
          .map((item) => item.id),
      ),
    [items],
  );
  ...
  const submitSupplier = (event: FormEvent) => {
    event.preventDefault();
    void run(
      () => addSupplier({ data: supplierForm }),
      "تمت إضافة المورد",
      () => setSupplierForm({ name: "", contact_name: "", email: "", phone: "", notes: "" }),
    );
  };
  const submitInvoice = (event: FormEvent) => {
    ...
    void run(
      () =>
        addInvoice({
          data: {
            contact_id: invoiceForm.relation_id,
            due_date: invoiceForm.date,
            description: invoiceForm.description,
            quantity: invoiceForm.quantity,
            unit_price: invoiceForm.price,
  ...
  const submitPurchase = (event: FormEvent) => {
    ...
    void run(
      () =>
        addPurchase({
          data: {
            supplier_id: purchaseForm.relation_id,
            description: purchaseForm.description,
            quantity: purchaseForm.quantity,
            unit_cost: purchaseForm.price,
            expected_date: purchaseForm.date,
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              العملاء والمخزون والفواتير والمبيعات والمشتريات في مكان واحد.
            </p>
          </div>
        ...
        {error ? (
          <div className="rounded-xl border border-warning/30 bg-warning/10 p-6 text-sm">
            <p className="font-semibold">
              الوحدة جاهزة في الكود وتحتاج تطبيق تحديثات قاعدة البيانات.
            </p>
          ...
          <>
            <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">
              ...
              <Stat
                label="قيمة المبيعات"
                value={`${Number(overview?.totals.salesValue ?? 0).toLocaleString("ar-EG")} ج.م`}
              />
              <Stat
                label="قيمة المشتريات"
                value={`${Number(overview?.totals.purchaseValue ?? 0).toLocaleString("ar-EG")} ج.م`}
              />
            </div>
            ...
            <Tabs defaultValue="crm" dir="rtl">
              <TabsList className="grid h-auto w-full grid-cols-2 gap-1 sm:grid-cols-3 lg:grid-cols-6">
                <TabsTrigger value="crm">
                  <ContactRound className="me-2 h-4 w-4" />
                  العملاء
                </TabsTrigger>
              ...
              <TabsContent value="crm" className="mt-5 space-y-5">
                <form
                  ...
                  className="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-2 lg:grid-cols-5"
                >
                ...
                <DataTable
                  headers={["الاسم", "الشركة", "الهاتف", "البريد", "الحالة"]}
                  empty="لا يوجد عملاء بعد"
                >
                  {contacts.map((x) => (
                    <tr key={x.id} className="border-t">
                      <Cell>{x.full_name}</Cell>
                      <Cell>{x.organization || "—"}</Cell>
                      <Cell>{x.phone || "—"}</Cell>
                      <Cell>{x.email || "—"}</Cell>
                      <Cell>{x.status}</Cell>
                    </tr>
              ...
              <TabsContent value="inventory" className="mt-5 space-y-5">
                <form
                  onSubmit={submitItem}
                  className="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-3 lg:grid-cols-7"
                >
                  ...
                  <NumberField
                    ...
                    onChange={(v) => setItemForm({ ...itemForm, minimum_quantity: v })}
                  />
                  <NumberField
                    label="التكلفة"
                    value={itemForm.unit_cost}
                    onChange={(v) => setItemForm({ ...itemForm, unit_cost: v })}
                  />
                  <NumberField
                    label="سعر البيع"
                ...
                <DataTable
                  headers={["الصنف", "SKU", "الكمية", "المتاح", "التكلفة", "سعر البيع"]}
                  empty="لا توجد أصناف بعد"
                >
                  {items.map((x) => (
                    <tr
                      key={x.id}
                      className={lowStockIds.has(x.id) ? "border-t bg-warning/5" : "border-t"}
                    >
                      <Cell>{x.name}</Cell>
                      <Cell>{x.sku || "—"}</Cell>
                      <Cell>{Number(x.quantity).toLocaleString("ar-EG")}</Cell>
                      <Cell>
                        {Math.max(
                          0,
                          Number(x.quantity) - Number(x.reserved_quantity),
                        ).toLocaleString("ar-EG")}
                      </Cell>
                      <Cell>{Number(x.unit_cost).toLocaleString("ar-EG")}</Cell>
                      <Cell>{Number(x.unit_price).toLocaleString("ar-EG")}</Cell>
                    </tr>
                  ))}
                </DataTable>
              </TabsContent>
              ...
              <TabsContent value="invoices" className="mt-5 space-y-5">
                <OrderForm
                  ...
                  relations={contacts.map((x) => ({ id: x.id, name: x.full_name }))}
                  relationLabel="العميل"
                  ...
                  extra={
                    <>
                      <NumberField
                        ...
                        onChange={(v) => setInvoiceForm({ ...invoiceForm, discount: v })}
                      />
                      <NumberField
                        label="الضريبة"
                        value={invoiceForm.tax}
                        onChange={(v) => setInvoiceForm({ ...invoiceForm, tax: v })}
                      />
                    </>
                  }
                />
                ...
                >
                  {invoices.map((x: any) => (
                    <tr key={x.id} className="border-t">
                      <Cell>{x.invoice_number}</Cell>
                      <Cell>{x.crm_contacts?.full_name || "—"}</Cell>
                      <Cell>{x.issue_date}</Cell>
                      <Cell>{x.due_date || "—"}</Cell>
                      <Cell>
                        {Number(x.total).toLocaleString("ar-EG")} {x.currency}
                      </Cell>
                      <Cell>{Number(x.paid_amount).toLocaleString("ar-EG")}</Cell>
                      <Cell>{x.status}</Cell>
                    </tr>
                  ))}
                </DataTable>
              ...
              <TabsContent value="sales" className="mt-5 space-y-5">
                ...
                />
                <DataTable
                  headers={["رقم الأمر", "العميل", "التاريخ", "القيمة", "الحالة"]}
                  empty="لا توجد أوامر بيع بعد"
                >
                  {sales.map((x: any) => (
                    <tr key={x.id} className="border-t">
                      <Cell>{x.order_number}</Cell>
                      <Cell>{x.crm_contacts?.full_name || "—"}</Cell>
                      <Cell>{x.order_date}</Cell>
                      <Cell>
                        {Number(x.total).toLocaleString("ar-EG")} {x.currency}
                      </Cell>
                      <Cell>{x.status}</Cell>
              ...
              <TabsContent value="suppliers" className="mt-5 space-y-5">
                ...
                <DataTable
                  headers={["المورد", "مسؤول التواصل", "الهاتف", "البريد", "الحالة"]}
                  empty="لا يوجد موردون بعد"
                >
                  {suppliers.map((x) => (
                    <tr key={x.id} className="border-t">
                      <Cell>{x.name}</Cell>
                      <Cell>{x.contact_name || "—"}</Cell>
                      <Cell>{x.phone || "—"}</Cell>
                      <Cell>{x.email || "—"}</Cell>
                      <Cell>{x.status}</Cell>
                    </tr>
              ...
              <TabsContent value="purchases" className="mt-5 space-y-5">
                <OrderForm
                  ...
                  relations={suppliers.map((x) => ({ id: x.id, name: x.name }))}
                  relationLabel="المورد"
                  onSubmit={submitPurchase}
                  busy={busy}
                />
                <DataTable
                  headers={["رقم الأمر", "المورد", "التاريخ", "المتوقع", "القيمة", "الحالة"]}
                  empty="لا توجد أوامر شراء بعد"
                >
                  {purchases.map((x: any) => (
                    <tr key={x.id} className="border-t">
                      <Cell>{x.order_number}</Cell>
                      <Cell>{x.business_suppliers?.name || "—"}</Cell>
                      <Cell>{x.order_date}</Cell>
                      <Cell>{x.expected_date || "—"}</Cell>
                      <Cell>
                        {Number(x.total).toLocaleString("ar-EG")} {x.currency}
                      </Cell>
                      <Cell>{x.status}</Cell>
...
function OrderForm({
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-2 lg:grid-cols-7"
    >
      <p className="col-span-full font-semibold">{title}</p>
...
function Stat({
  ...
  return (
    <div className="rounded-xl border bg-card p-4">
      ...
      <p className={danger ? "mt-2 text-xl font-bold text-warning" : "mt-2 text-xl font-bold"}>
        {value}
      </p>
    </div>
  );
