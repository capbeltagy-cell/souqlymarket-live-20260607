  ...
  const act = async (action: "accept" | "reject") => {
    ...
    try {
      const res = await respond({
        data: { id, action, shipping_address: action === "accept" ? addr : null },
      });
      toast.success(action === "accept" ? "تم القبول وإنشاء الطلب" : "تم الرفض");
      if ((res as any).order_id)
        navigate({ to: "/orders/$id", params: { id: (res as any).order_id } });
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-card">
            ...
            {items.map((it) => (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {it.quantity} × {Number(it.unit_price).toLocaleString("ar-EG")} {q.currency}
                    {Number(it.discount) > 0 && ` - خصم ${it.discount}`}
                  </div>
                </div>
                <div className="font-semibold">
                  {Number(it.total).toLocaleString("ar-EG")} {q.currency}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-lg bg-muted p-4 mb-4 space-y-1 text-sm">
            <div className="flex justify-between">
              ...
              <span>
                {Number(q.subtotal).toLocaleString("ar-EG")} {q.currency}
              </span>
            </div>
            {Number(q.discount) > 0 && (
              <div className="flex justify-between text-destructive">
                <span>خصم</span>
                <span>-{Number(q.discount).toLocaleString("ar-EG")}</span>
              </div>
            )}
            {Number(q.tax) > 0 && (
              <div className="flex justify-between">
                <span>ضريبة</span>
                <span>+{Number(q.tax).toLocaleString("ar-EG")}</span>
              </div>
            )}
            {Number(q.shipping) > 0 && (
              <div className="flex justify-between">
                <span>شحن</span>
                <span>+{Number(q.shipping).toLocaleString("ar-EG")}</span>
              </div>
            )}
            <div className="flex justify-between text-lg font-bold pt-2 border-t border-border">
              <span>الإجمالي</span>
              <span>
                {Number(q.total).toLocaleString("ar-EG")} {q.currency}
              </span>
            </div>
          </div>

          {q.expiry_date && (
            <div className="text-sm text-muted-foreground mb-2">صالح حتى: {q.expiry_date}</div>
          )}
          {q.notes && (
            <div className="rounded-md border border-border p-3 mb-4 text-sm bg-background/50">
              {q.notes}
            </div>
          )}
          ...
          {q.status === "converted" && q.order_id && (
            <Button asChild className="w-full">
              <Link to="/orders/$id" params={{ id: q.order_id }}>
                عرض الطلب →
              </Link>
            </Button>
          )}
              ...
              <div className="rounded-lg border border-border p-3 bg-background/50">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setAddr({ ...addr, governorate: e.target.value })}
                  />
                  <Input
                    placeholder="المدينة"
                    value={addr.city}
