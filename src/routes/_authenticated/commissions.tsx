...
function CommissionsPage() {
  ...
  const onRequestPayout = async (id: string) => {
    try {
      await reqPayout({ data: { id } });
      toast.success(t("payout_requested") || "Payout requested");
      load();
    } catch (e) {
      toast.error((e as Error).message);
    }
      ...
      <div className="container-souqly py-8 flex-1">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => downloadCsv(rows, locale)}
            >
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <KPI
            ...
            value={formatPrice(totals.total, locale, { showZero: true })}
            icon={DollarSign}
          />
          <KPI
            label={t("commissions_paid")}
            value={formatPrice(totals.paid, locale, { showZero: true })}
            icon={CheckCircle2}
          />
          <KPI
            label={t("commissions_pending")}
            value={formatPrice(totals.pending, locale, { showZero: true })}
            icon={Clock}
          />
        </div>
                ...
                <div key={r.id} className="p-4 flex items-center justify-between gap-4 flex-wrap">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      ...
                      {r.payout_requested_at && (
                        <> · 📨 {new Date(r.payout_requested_at).toLocaleDateString()}</>
                      )}
                      {r.paid_at && <> · ✅ {new Date(r.paid_at).toLocaleDateString()}</>}
                    </div>
                  </div>
                  <div className="font-bold text-success">
                    {formatPrice(Number(r.amount), locale)}
                  </div>
...
function StatusBadge({ status }: { status: string }) {
  const v =
    status === "paid"
      ? { c: "bg-success/10 text-success", I: CheckCircle2 }
      : status === "approved"
        ? { c: "bg-primary/10 text-primary", I: CheckCircle2 }
        : status === "pending"
          ? { c: "bg-muted text-muted-foreground", I: Clock }
          : { c: "bg-destructive/10 text-destructive", I: XCircle };
  const Icon = v.I;
  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs ${v.c}`}>
