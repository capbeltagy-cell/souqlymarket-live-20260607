...
const empty: Form = { full_name: "", display_name: "", phone: "", bio: "", avatar_url: "", preferred_language: "ar" };
...
function ProfilePage() {
  ...
  const uploadAvatar = async (file: File) => {
    if (!user) return;
    if (file.size > 5 * 1024 * 1024) { toast.error(msg("الحد الأقصى 5 ميجا", "Max size is 5MB")); return; }
    setUploading(true);
    try {
      ...
      const { data, error: sErr } = await supabase.storage.from("avatars").createSignedUrl(path, 60 * 60 * 24 * 365 * 10);
  ...
  return (
    <Shell>
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={save} className="rounded-xl border border-border bg-card p-4 sm:p-6 shadow-card space-y-5">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 pb-5 border-b border-border">
            <div className="relative">
              {form.avatar_url ? (
                <img src={form.avatar_url} alt="" className="h-20 w-20 rounded-full object-cover border border-border" />
              ) : (
                <div className="h-20 w-20 rounded-full bg-primary text-primary-foreground grid place-items-center text-2xl font-bold">{initial}</div>
              )}
              <button
                type="button"
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {isCompany && (
                <Button asChild type="button" variant="outline" className="justify-start gap-2">
                  <Link to="/company"><Building2 className="h-4 w-4" />{msg("ملف الشركة", "Company profile")}</Link>
                </Button>
              )}
              {isAgent && (
                <Button asChild type="button" variant="outline" className="justify-start gap-2">
                  <Link to="/agent"><UserCircle2 className="h-4 w-4" />{msg("ملف المسوق", "Marketer profile")}</Link>
                </Button>
              )}
            </div>
          )}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} dir={ar ? "rtl" : "ltr"} />
            </div>
            <div className="space-y-2">
              <Label>{t("profile_display_name")}</Label>
              <Input value={form.display_name} onChange={(e) => setForm({ ...form, display_name: e.target.value })} placeholder={form.full_name} dir={ar ? "rtl" : "ltr"} />
            </div>
            <div className="space-y-2">
              <Label>{t("profile_phone")}</Label>
              <Input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="+20 1X XXX XXXX" inputMode="tel" />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Textarea rows={4} value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} dir={ar ? "rtl" : "ltr"} />
          </div>

          <div className="flex flex-col-reverse sm:flex-row gap-2 pt-2">
            <Button asChild type="button" variant="ghost"><Link to="/dashboard">{t("nav_dashboard")}</Link></Button>
            <Button type="submit" disabled={saving} className="bg-primary hover:bg-primary-hover sm:ms-auto">
