...
function AchievementsPage() {
  ...
  useEffect(() => {
    fetchIt().then((r) => setRows(r.achievements));
  }, []);
      ...
      <div className="container-souqly py-8 flex-1">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-2xl font-bold">
            {ar ? "الإنجازات والشارات" : "Achievements & badges"}
          </h1>
        </div>
        {rows.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border bg-card p-10 text-center text-muted-foreground">
            {ar
              ? "لا توجد شارات بعد. أكمل صفقاتك لكسبها!"
              : "No badges yet. Close deals to unlock them!"}
          </div>
        ) : (
            ...
            {rows.map((a) => (
              <div key={a.id} className="rounded-lg border border-border bg-card p-5 shadow-card">
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  {ar ? a.description_ar : a.description_en}
                </div>
