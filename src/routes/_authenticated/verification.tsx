...
import { listVerificationQueue, setCompanyVerified, setAgentVerified } from "@/lib/verification.functions";
...
function VerificationPage() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try { const res = await fetchQueue(); setCompanies(res.companies); setAgents(res.agents); }
  ...
  const toggleC = async (id: string, v: boolean) => {
    try { await verifyCompany({ data: { id, verified: v } }); toast.success(t("status_updated")); load(); }
    catch (e) { toast.error((e as Error).message); }
  };
  const toggleA = async (id: string, v: boolean) => {
    try { await verifyAgent({ data: { id, verified: v } }); toast.success(t("status_updated")); load(); }
    catch (e) { toast.error((e as Error).message); }
  };
    ...
    <div className="min-h-screen flex flex-col bg-surface-2">
      ...
      <div className="container-souqly py-8 flex-1">
        <h1 className="text-2xl font-bold flex items-center gap-2 mb-6">
          <ShieldCheck className="h-6 w-6 text-primary" />{t("verification_title")}
        </h1>
        ...
        {loading ? (
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <Panel title={t("nav_companies")} icon={Building2}>
              {companies.length === 0 ? <Empty /> : companies.map((c) => (
                <Row key={c.id}
            ...
            <Panel title={t("nav_agents")} icon={UserIcon}>
              {agents.length === 0 ? <Empty /> : agents.map((a) => (
                <Row key={a.id}
  ...
  function Row({ title, sub, verified, onToggle }: { title: string; sub: string; verified: boolean; onToggle: (v: boolean) => void }) {
    return (
      <div className="p-4 flex items-center gap-3 border-b border-border last:border-0">
        ...
        <Button size="sm" variant={verified ? "outline" : "default"} className={verified ? "" : "bg-primary hover:bg-primary-hover"}
