...
function NewQuotationPage() {
  ...
  const [items, setItems] = useState<Item[]>([
    { title: "", quantity: 1, unit_price: 0, discount: 0 },
  ]);
  ...
  const subtotal = items.reduce(
    (s, i) => s + Math.max(0, i.quantity * i.unit_price - (i.discount || 0)),
    0,
  );
      ...
      <section className="container-souqly py-8 max-w-3xl mx-auto w-full flex-1">
        ...
        <div className="space-y-4">
          {items.map((it, idx) => (
            <div key={idx} className="rounded-lg border border-border p-4 bg-card grid gap-2">
              ...
              <div className="text-xs text-muted-foreground text-end">
                إجمالي البند:{" "}
                {Math.max(0, it.quantity * it.unit_price - it.discount).toLocaleString("ar-EG")}{" "}
                {currency}
              </div>
            </div>
          ))}
          <Button
            variant="outline"
            onClick={() =>
              setItems([...items, { title: "", quantity: 1, unit_price: 0, discount: 0 }])
            }
          >
          ...
          <div className="rounded-lg border-2 border-primary bg-primary/5 p-4 flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              <div>
                المجموع الفرعي: {subtotal.toLocaleString("ar-EG")} {currency}
              </div>
              <div>
                خصم: -{discount.toLocaleString("ar-EG")} | ضريبة: +{tax.toLocaleString("ar-EG")} |
                شحن: +{shipping.toLocaleString("ar-EG")}
              </div>
            </div>
