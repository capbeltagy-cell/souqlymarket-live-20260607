  ...
  const onChange = async (lead: Lead, status: string) => {
    ...
    try {
      await setStatus({
        data: {
          leadId: lead.id,
          status: status as "new" | "contacted" | "negotiating" | "won" | "lost",
        },
      });
      ...
      <div className="container-souqly py-8 flex-1">
        ...
        {!leads ? (
          ...
          <div className="rounded-lg border border-dashed border-border bg-card p-12 text-center text-muted-foreground">
            {ar
              ? "لا توجد طلبات بعد. شارك إعلاناتك لتصل لعملاء أكثر."
              : "No leads yet. Share your listings to reach more buyers."}
          </div>
        ) : (
          <div className="grid gap-3">
            {leads.map((l) => {
              ...
              const title =
                (ar ? li?.title_ar : li?.title_en) ?? li?.title_en ?? li?.title_ar ?? "—";
              return (
                <div key={l.id} className="rounded-lg border border-border bg-card p-4 shadow-card">
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="min-w-0">
                      ...
                      {l.message && (
                        <div className="mt-2 text-sm text-foreground flex gap-2">
