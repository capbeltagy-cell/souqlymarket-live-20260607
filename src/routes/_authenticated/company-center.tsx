...
function CommandCenter() {
      ...
      <Shell>
        <div className="rounded-xl border border-primary/30 bg-primary/5 p-6 text-center">
          ...
          <h2 className="text-xl font-bold mb-2">
            {ar ? "أنشئ ملف شركتك أولاً" : "Create your company first"}
          </h2>
          <p className="text-sm text-muted-foreground mb-4">
            {ar
              ? "لتتمكن من إدارة المنتجات والحملات والعمولات."
              : "So you can manage products, campaigns and commissions."}
          </p>
          <Button
            onClick={() => navigate({ to: "/company" })}
            className="bg-primary hover:bg-primary-hover"
          >
            {ar ? "إنشاء الشركة" : "Create company"}
          </Button>
        </div>
      </Shell>
    );
  ...
  return (
    <Shell>
        ...
        <div className="flex items-center gap-2">
          <Button asChild variant="outline" size="sm" className="gap-1">
            <Link to="/companies/$id" params={{ id: c.id }}>
              ...
              {ar ? "عرض الملف العام" : "Public profile"}
            </Link>
          </Button>
        </div>
      </div>
      ...
      {/* Subscription banner */}
      <div
        className={`rounded-xl border p-4 mb-6 ${d.subscription.isPaid ? "border-success/40 bg-success/5" : "border-primary/30 bg-primary/5"}`}
      >
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            <Crown
              className={`h-5 w-5 ${d.subscription.isPaid ? "text-success" : "text-primary"}`}
            />
            <div>
          ...
          {!d.subscription.isPaid && (
            <Button asChild size="sm" className="bg-primary hover:bg-primary-hover gap-1">
              <Link to="/subscribe">
      ...
      {d.profileCompletion.pct < 100 && (
        <div className="rounded-xl border border-border bg-card p-5 shadow-card mb-6">
          <div className="flex items-center justify-between mb-3">
            <div>
              <div className="font-semibold">
                {ar ? "قائمة إعداد الشركة" : "Company setup checklist"}
              </div>
              <div className="text-xs text-muted-foreground">
                {ar
                  ? `${d.profileCompletion.done} من ${d.profileCompletion.total} مكتملة`
                  : `${d.profileCompletion.done} of ${d.profileCompletion.total} complete`}
              </div>
            </div>
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {d.checklist.map((item) => (
              <Link
                key={item.key}
                to={item.ctaTo}
                className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm transition ${item.done ? "border-success/30 bg-success/5 text-muted-foreground" : "border-border hover:border-primary/40 hover:bg-primary/5"}`}
              >
                {item.done ? (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <KPI
          ...
          value={k.activeProducts}
        />
        <KPI
          icon={FileText}
          label={ar ? "خدمات نشطة" : "Active services"}
          value={k.activeServices}
        />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <KPI
          icon={Megaphone}
          label={ar ? "منتجات معروضة للتسويق" : "Promoted products"}
          value={k.activePromotedListings}
        />
        <KPI
          icon={Sparkles}
          label={ar ? "حملات مسوقين نشطة" : "Active campaigns"}
          value={k.activeCampaigns}
        />
        <KPI
          icon={Users}
          label={ar ? "مسوقون مشاركون" : "Marketers"}
          value={k.participatingMarketers}
        />
        <KPI
          icon={Activity}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        <KPI
          icon={BarChart3}
          label={ar ? "نقرات الروابط" : "Referral clicks"}
          value={k.referralClicks}
        />
        <KPI
          icon={BarChart3}
          label={ar ? "تحويلات" : "Conversions"}
          value={k.referralConversions}
        />
        <KPI
          icon={DollarSign}
          label={ar ? "عمولات مولّدة" : "Commissions generated"}
          value={`${k.totalCommissionsEgp.toFixed(0)} ج.م`}
        />
        <KPI
          icon={DollarSign}
          label={ar ? "عمولات معلّقة" : "Pending commissions"}
          value={`${k.pendingCommissionsEgp.toFixed(0)} ج.م`}
        />
      </div>
            ...
            {d.activity.map((a) => (
              <div key={a.id} className="p-3 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <span
                    className={`h-8 w-8 rounded-lg grid place-items-center shrink-0 ${a.kind === "lead" ? "bg-primary/10 text-primary" : "bg-success/10 text-success"}`}
                  >
                    {a.kind === "lead" ? (
                      <Inbox className="h-4 w-4" />
                    ) : (
                ...
                {a.link && (
                  <Link to={a.link} className="text-xs text-primary hover:underline shrink-0">
                    {ar ? "عرض" : "View"}
                  </Link>
                )}
              </div>
