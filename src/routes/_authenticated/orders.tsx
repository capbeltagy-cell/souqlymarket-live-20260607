...
type Order = {
  ...
  _listing?: { id: string; title_ar?: string | null; title_en?: string | null; images?: string[] | null } | null;
  _company?: { id: string; name_ar?: string | null; name_en?: string | null; logo_url?: string | null } | null;
};
...
const FILTERS: Array<{ key: string; label: string; match: (s: string) => boolean }> = [
  ...
  { key: "cancelled", label: "ملغي/مرفوض", match: (s) => s === "cancelled" || s === "rejected" || s === "returned" },
];
...
function OrdersPage() {
  ...
  const applyFilters = (orders: Order[]) => {
    ...
    return orders.filter((o) => {
      ...
      if (!ql) return true;
      const hay = `${o.id} ${o._listing?.title_ar ?? ""} ${o._listing?.title_en ?? ""} ${o._company?.name_ar ?? ""} ${o._company?.name_en ?? ""}`.toLowerCase();
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section className="container-souqly py-6 sm:py-8 flex-1">
        <h1 className="text-2xl font-bold mb-4 flex items-center gap-2"><ShoppingBag className="h-6 w-6" />طلباتي</h1>
        ...
        <Tabs defaultValue="buyer">
          ...
          <TabsContent value="buyer" className="mt-4">
            {loading ? <div className="text-sm text-muted-foreground">جارٍ التحميل…</div>
              : filteredBuyer.length === 0 ? <EmptyState icon={<Package className="h-7 w-7" />} title="لا توجد طلبات" description="لا توجد نتائج مطابقة" />
              : <OrderList orders={filteredBuyer} />}
          </TabsContent>
          <TabsContent value="seller" className="mt-4">
            {loading ? <div className="text-sm text-muted-foreground">جارٍ التحميل…</div>
              : filteredSeller.length === 0 ? <EmptyState icon={<Truck className="h-7 w-7" />} title="لا توجد مبيعات" description="لا توجد نتائج مطابقة" />
  ...
  return (
    <div className="grid gap-3">
      {orders.map((o) => {
        ...
        return (
          <Link key={o.id} to="/orders/$id" params={{ id: o.id }} className="block rounded-lg border border-border bg-card p-3 sm:p-4 hover:border-primary transition">
            <div className="flex items-start gap-3">
              {img && <img src={img} alt="" className="h-14 w-14 sm:h-16 sm:w-16 rounded object-cover shrink-0" />}
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 flex-wrap">
                  <div className="min-w-0">
                    <div className="font-semibold truncate">{title}</div>
                    {company && <div className="text-xs text-muted-foreground truncate">{company}</div>}
                    <div className="text-[11px] text-muted-foreground mt-0.5">#{o.id.slice(0, 8)} • {new Date(o.created_at).toLocaleString("ar-EG")}</div>
                  </div>
                ...
                <div className="flex gap-1.5 flex-wrap mt-2">
                  {o.payment_status === "paid" && <Badge variant="secondary" className="text-[10px]">مدفوع</Badge>}
                  {o.payment_status === "pending_review" && <Badge variant="secondary" className="text-[10px]">مراجعة الدفع</Badge>}
                  {o.tracking_number && <Badge variant="secondary" className="text-[10px]">شحنة: {o.tracking_number}</Badge>}
                  {o.referral_code && <Badge variant="secondary" className="text-[10px]">إحالة</Badge>}
                </div>
              </div>
            </div>
          </Link>
