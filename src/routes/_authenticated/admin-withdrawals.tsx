...
import { Banknote, Check, X, DollarSign, Settings2 } from "lucide-react";
    ...
    <div className="min-h-screen flex flex-col bg-surface-2">
      ...
      <div className="container-souqly py-8 flex-1 space-y-6">
        ...
        <div className="rounded-lg border border-border bg-card shadow-card overflow-hidden">
          {rows.length === 0 ? <div className="p-8 text-center text-sm text-muted-foreground">{ar ? "لا توجد طلبات" : "No requests"}</div> :
           <div className="divide-y divide-border">
             {rows.map((p) => {
               const name = p.profiles?.display_name || p.profiles?.full_name || p.user_id?.slice(0, 8);
               const detailStr = Object.entries(p.payout_methods?.details ?? {}).map(([k, v]) => `${k}: ${v}`).join(" · ");
               return (
                 <div key={p.id} className="p-4 space-y-2">
                   <div className="flex flex-wrap items-center justify-between gap-2">
                     <div>
                       <div className="font-semibold">{formatPrice(Number(p.amount), locale, { showZero: true })}</div>
                       <div className="text-xs text-muted-foreground">{name} · {new Date(p.created_at).toLocaleString()}</div>
                     </div>
                     <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground">{p.status}</span>
                   </div>
                   ...
                   {(p.status === "pending" || p.status === "approved") && (
                     <div className="flex flex-wrap items-center gap-2 pt-1">
                       <Input placeholder={ar ? "ملاحظة الإدارة (اختياري)" : "Admin note (optional)"} value={notesMap[p.id] ?? ""} onChange={(e) => setNotesMap({ ...notesMap, [p.id]: e.target.value })} className="max-w-xs h-8 text-xs" />
                       {p.status === "pending" && (
                         <>
                           <Button size="sm" onClick={() => act(p.id, "approve")} disabled={busy === p.id + "approve"} className="gap-1"><Check className="h-4 w-4" />{ar ? "اعتماد" : "Approve"}</Button>
                           <Button size="sm" variant="destructive" onClick={() => act(p.id, "reject")} disabled={busy === p.id + "reject"} className="gap-1"><X className="h-4 w-4" />{ar ? "رفض" : "Reject"}</Button>
                         </>
                       )}
                       <Button size="sm" variant="outline" onClick={() => act(p.id, "paid")} disabled={busy === p.id + "paid"} className="gap-1"><DollarSign className="h-4 w-4" />{ar ? "تم الدفع" : "Mark paid"}</Button>
                     </div>
                   )}
                 </div>
               );
