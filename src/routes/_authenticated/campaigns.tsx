...
function CampaignsPage() {
  ...
  const toggle = async (id: string, status: string) => {
    ...
    try {
      await fUpdate({ data: { id, status: next as any } });
      load();
    } catch (e) {
      toast.error((e as Error).message);
    }
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <div className="md:col-span-2">
              <Label>{ar ? "الإعلان (اختياري)" : "Listing (optional)"}</Label>
              <select
                value={listingId}
                  ...
                  <div key={c.id} className="p-4 flex flex-col sm:flex-row sm:items-center gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground mt-1">
                        {ar ? "نقرات" : "Clicks"} {clicks} · {ar ? "تحويلات" : "Conv"} {conv} ·{" "}
                        {refs.length} {ar ? "روابط" : "links"}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="ghost" onClick={() => toggle(c.id, c.status)}>
                        {c.status === "active" ? (
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${cls}`}>{status}</span>
  );
}
