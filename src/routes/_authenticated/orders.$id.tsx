  ...
  const reload = () =>
    load({ data: { id } })
      .then((r) => {
        ...
        setCarrier(r.order.tracking_carrier ?? "");
      })
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-card">
          <div className="flex items-start justify-between gap-3 flex-wrap mb-6">
            ...
            <div className="text-end">
              <div className="text-2xl font-bold">
                {Number(order.total_amount ?? 0).toLocaleString("ar-EG")} {order.currency ?? "EGP"}
              </div>
          ...
          {listing && (
            <Link
              ...
              className="flex items-center gap-3 rounded-lg border border-border p-3 mb-6 hover:border-primary transition"
            >
              {listing.images?.[0] && (
                <img src={listing.images[0]} alt="" className="h-16 w-16 rounded object-cover" />
              )}
              ...
              {TIMELINE.map((step, i) => {
                ...
                return (
                  <div key={step.key} className="flex flex-col items-center flex-1">
                    <div
                      className={`h-9 w-9 rounded-full grid place-items-center border-2 ${done ? "bg-primary border-primary text-primary-foreground" : "bg-muted border-border text-muted-foreground"}`}
                    >
                      <Icon className="h-4 w-4" />
                    </div>
                    <div
                      className={`text-[10px] mt-1 text-center ${done ? "font-semibold" : "text-muted-foreground"}`}
                    >
                      {step.label}
                    </div>
                  </div>
          ...
          {company && (
            <Link
              ...
              className="flex items-center gap-3 rounded-lg border border-border p-3 mb-4 hover:border-primary transition"
            >
              {company.logo_url && (
          ...
          {order.referral_code && (
            <div className="rounded-md bg-primary/10 border border-primary/40 p-3 mb-4 text-sm flex items-center gap-2">
              ...
              <span>
                هذا الطلب مرتبط بكود إحالة تسويقية{" "}
                <span className="font-mono">{order.referral_code}</span>
              </span>
            </div>
          )}
          ...
          {order.tracking_number && (
            <div className="rounded-lg border border-border p-3 mb-4 text-sm">
              ...
              <div className="text-muted-foreground">
                {order.tracking_carrier ?? "شركة الشحن"} — رقم:{" "}
          ...
          <div className="border-t border-border pt-4 flex flex-wrap gap-2">
            {isBuyer &&
              ...
              ["accepted", "awaiting_seller", "packed"].includes(order.status) && (
            ...
            {order.payment_status === "paid" && (
              <Badge className="bg-success text-white">
                <ShieldCheck className="h-3 w-3 me-1" /> مدفوع — محفوظ لدى المنصة
              </Badge>
            )}
            ...
            {!isBuyer && order.status === "awaiting_seller" && (
              <>
                <Button
                  onClick={() => act({ id: order.id, status: "accepted" })}
                  disabled={busy}
                ...
                <Button
                  onClick={() =>
                    act({
                      id: order.id,
                      status: "rejected",
                      cancelled_reason: "تم الرفض من البائع",
                    })
                  }
                  disabled={busy}
                  variant="outline"
            ...
            {!isBuyer && order.status === "accepted" && (
              <Button
                onClick={() => act({ id: order.id, status: "packed" })}
                disabled={busy}
                className="bg-primary hover:bg-primary-hover"
              >
                تم التجهيز
            ...
            {!isBuyer && order.status === "shipped" && (
              <Button
                onClick={() => act({ id: order.id, status: "delivered" })}
                disabled={busy}
                className="bg-primary hover:bg-primary-hover"
              >
                تأكيد التسليم
            ...
            {isBuyer && order.status === "delivered" && (
              <Button
                onClick={() => act({ id: order.id, status: "completed" })}
                disabled={busy}
              ...
              <Button
                onClick={() =>
                  act({
                    id: order.id,
                    status: "returned",
                    cancelled_reason: "طلب إرجاع من المشتري",
                  })
                }
                disabled={busy}
                variant="outline"
            ...
            {(order.status === "awaiting_seller" || order.status === "accepted") && isBuyer && (
              <Button
                onClick={() =>
                  act({ id: order.id, status: "cancelled", cancelled_reason: "ملغي من المشتري" })
                }
                disabled={busy}
                variant="outline"
              >
