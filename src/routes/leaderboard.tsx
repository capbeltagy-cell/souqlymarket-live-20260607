...
function LeaderboardPage() {
  ...
  useEffect(() => {
    fetchIt().then((r) => setRows(r.leaders));
  }, []);
    ...
    <div className="min-h-screen flex flex-col bg-surface-2">
      ...
      <div className="container-souqly py-8 flex-1 max-w-4xl">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-3xl font-bold">{ar ? "لوحة المتصدرين" : "Leaderboard"}</h1>
        </div>
                ...
                <div key={r.agent_id} className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {r.deals_closed} {ar ? "صفقة" : "deals"} · {r.achievements_count}{" "}
                      {ar ? "إنجازات" : "badges"}
                    </div>
                  </div>
                  <div className="text-primary font-bold">
                    {formatPrice(Number(r.total_earned), locale, { showZero: true })}
                  </div>
                </div>
              ))}
            </div>
