      ...
      <section className="container-souqly py-10 flex-1">
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr] items-center mb-8">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-2">{ar ? "انشر مناقصة واستقبل عروض الموردين" : "Publish a tender and receive supplier proposals"}</p>
          </div>
          <div className="flex flex-wrap justify-end gap-3">
            ...
            <div className="rounded-3xl bg-surface px-4 py-3 text-sm text-muted-foreground">{rows.length} {ar ? "مناقصة" : "tenders"}</div>
          </div>
        </div>
        ...
        <div className="grid gap-4">
          ...
          {rows.map((t) => (
            <Link key={t.id} to="/tenders/$id" params={{ id: t.id }} className="block rounded-[1.5rem] border border-border bg-surface-2 p-6 hover:bg-surface shadow-elev transition">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mt-4 text-sm text-muted-foreground">
                    {t.category_slug && <span className="rounded-2xl bg-white/5 px-3 py-1">{t.category_slug}</span>}
                    {t.governorate && <span className="rounded-2xl bg-white/5 px-3 py-1">{t.governorate}</span>}
                ...
                <div className="flex flex-col gap-3 items-start sm:items-end">
                  ...
                  <div className="text-sm text-muted-foreground">{ar ? "قدم على العرض قبل:" : "Apply by:"} {t.deadline ?? "—"}</div>
                </div>
              </div>
            </Link>
          ))}
