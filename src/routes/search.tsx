    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="container-souqly py-10 flex-1">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
          ...
          <select className="h-10 rounded-md border border-input bg-background px-3" value={verified} onChange={(e) => setVerified(e.target.value as any)}>
          ...
          <select className="h-10 rounded-md border border-input bg-background px-3" value={plan} onChange={(e) => setPlan(e.target.value as any)}>
          ...
          <Button onClick={run} className="bg-primary hover:bg-primary-hover">{ar ? "بحث" : "Search"}</Button>
        </div>
        {loading ? <div className="text-center py-10">…</div> : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {results.map((c) => (
              <Link key={c.id} to="/companies/$id" params={{ id: c.id }} className="rounded-lg border border-border bg-card p-5 hover:bg-muted shadow-card">
                <div className="font-semibold">{ar ? c.name_ar : c.name_en}</div>
                <div className="text-sm text-muted-foreground">{[c.city, c.governorate, c.country].filter(Boolean).join(" · ")}</div>
                <div className="flex gap-2 mt-2 flex-wrap">
                  {c.is_verified && <span className="text-xs rounded px-2 py-0.5 bg-primary/10 text-primary">{ar ? "موثقة" : "Verified"}</span>}
                  {c.subscription_plan === "paid" && <span className="text-xs rounded px-2 py-0.5 bg-success/10 text-success">{ar ? "مدفوع" : "Paid"}</span>}
                  {c.company_type && <span className="text-xs rounded px-2 py-0.5 bg-muted">{c.company_type}</span>}
                </div>
              </Link>
            ))}
            {!loading && results.length === 0 && <div className="col-span-full text-center text-muted-foreground py-10">{ar ? "لا نتائج" : "No results"}</div>}
          </div>
        )}
      </section>
      <SiteFooter />
